    ...
    fn get_kind(&self) -> Option<IdentifierKind<Str>> {
        match self.indices.as_slice() {
            [] => match self.symbol.inner().as_str() {
                ...
                IMPLIES => Some(IdentifierKind::Implies),
                ...
                SUB => Some(IdentifierKind::Sub),
                MUL => Some(IdentifierKind::Mul),
                IDIV => Some(IdentifierKind::Idiv),
                RDIV => Some(IdentifierKind::Rdiv),
                MOD => Some(IdentifierKind::Mod),
                ABS => Some(IdentifierKind::Abs),
                LE => Some(IdentifierKind::Le),
                LT => Some(IdentifierKind::Lt),
                GE => Some(IdentifierKind::Ge),
                GT => Some(IdentifierKind::Gt),
                TO_REAL => Some(IdentifierKind::ToReal),
                TO_INT => Some(IdentifierKind::ToInt),
                IS_INT => Some(IdentifierKind::IsInt),
                STR_CONCAT => Some(IdentifierKind::StrConcat),
                STR_LEN => Some(IdentifierKind::StrLen),
                STR_LT => Some(IdentifierKind::StrLt),
                STR_TO_RE => Some(IdentifierKind::StrToRe),
                STR_IN_RE => Some(IdentifierKind::StrInRe),
                RE_NONE => Some(IdentifierKind::ReNone),
                RE_ALL => Some(IdentifierKind::ReAll),
                RE_ALLCHAR => Some(IdentifierKind::ReAllChar),
                RE_CONCAT => Some(IdentifierKind::ReConcat),
                RE_UNION => Some(IdentifierKind::ReUnion),
                RE_INTER => Some(IdentifierKind::ReInter),
                RE_STAR => Some(IdentifierKind::ReStar),
                STR_LE => Some(IdentifierKind::StrLe),
                STR_AT => Some(IdentifierKind::StrAt),
                STR_SUBSTR => Some(IdentifierKind::StrSubstr),
                STR_PREFIXOF => Some(IdentifierKind::StrPrefixof),
                STR_SUFFIXOF => Some(IdentifierKind::StrSuffixof),
                STR_CONTAINS => Some(IdentifierKind::StrContains),
                STR_INDEXOF => Some(IdentifierKind::StrIndexof),
                STR_REPLACE => Some(IdentifierKind::StrReplace),
                STR_REPLACE_ALL => Some(IdentifierKind::StrReplaceAll),
                STR_REPLACE_RE => Some(IdentifierKind::StrReplaceRe),
                STR_REPLACE_RE_ALL => Some(IdentifierKind::StrReplaceReAll),
                RE_COMP => Some(IdentifierKind::ReComp),
                RE_DIFF => Some(IdentifierKind::ReDiff),
                RE_ADD => Some(IdentifierKind::ReAdd),
                RE_OPT => Some(IdentifierKind::ReOpt),
                RE_RANGE => Some(IdentifierKind::ReRange),
                STR_IS_DIGIT => Some(IdentifierKind::StrIsDigit),
                STR_TO_CODE => Some(IdentifierKind::StrToCode),
                STR_FROM_CODE => Some(IdentifierKind::StrFromCode),
                STR_TO_INT => Some(IdentifierKind::StrToInt),
                STR_FROM_INT => Some(IdentifierKind::StrFromInt),
                BV_CONCAT => Some(IdentifierKind::Concat),
                BV_NOT => Some(IdentifierKind::BvNot),
                BV_NEG => Some(IdentifierKind::BvNeg),
                BV_AND => Some(IdentifierKind::BvAnd),
                BV_OR => Some(IdentifierKind::BvOr),
                BV_ADD => Some(IdentifierKind::BvAdd),
                BV_MUL => Some(IdentifierKind::BvMul),
                BV_UDIV => Some(IdentifierKind::BvUdiv),
                BV_UREM => Some(IdentifierKind::BvUrem),
                BV_SHL => Some(IdentifierKind::BvShl),
                BV_LSHR => Some(IdentifierKind::BvLshr),
                BV_ULT => Some(IdentifierKind::BvUlt),
                BV_NEGO => Some(IdentifierKind::BvNego),
                BV_UADDO => Some(IdentifierKind::BvUaddo),
                BV_SADDO => Some(IdentifierKind::BvSaddo),
                BV_UMULO => Some(IdentifierKind::BvUmulo),
                BV_SMULO => Some(IdentifierKind::BvSmulo),
                ...
                BV_SREM => Some(IdentifierKind::BvSrem),
                BV_SMOD => Some(IdentifierKind::BvSmod),
                BV_ASHR => Some(IdentifierKind::BvAShr),
                BV_USUBO => Some(IdentifierKind::BvUsubo),
                BV_SSUBO => Some(IdentifierKind::BvSsubo),
                BV_SDIVO => Some(IdentifierKind::BvSdivo),
                BV_ULE => Some(IdentifierKind::BvUle),
                BV_UGT => Some(IdentifierKind::BvUgt),
                BV_UGE => Some(IdentifierKind::BvUge),
                BV_SLT => Some(IdentifierKind::BvSlt),
            ...
            [Index::Numeral(n)] => match self.symbol.inner().as_str() {
                RE_POWER => Some(IdentifierKind::RePower(n.clone())),
                ...
                BV_REPEAT => Some(IdentifierKind::Repeat(n.clone())),
                BV_ZERO_EXTEND => Some(IdentifierKind::ZeroExtend(n.clone())),
            ...
            [Index::Numeral(l), Index::Numeral(h)] => match self.symbol.inner().as_str() {
                RE_LOOP => Some(IdentifierKind::ReLoop(l.clone(), h.clone())),
                BV_EXTRACT => Some(IdentifierKind::Extract(l.clone(), h.clone())),
                _ => None,
            },
            _ => None,
        }
